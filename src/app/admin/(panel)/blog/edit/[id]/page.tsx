...
export default function EditBlogPage() {
  ...
  const { posts, updatePost, isLoaded } = useBlogPosts();
  ...
  useEffect(() => {
    if (isLoaded && postId) {
      ...
      if (foundPost) {
        ...
        setFeaturedImage(foundPost.imageUrl);
        // In a real app, content images would be stored with the post
      } else {
        toast({ title: 'Post not found', variant: 'destructive' });
        router.push('/admin/blog');
      }
    }
  }, [postId, posts, isLoaded, router, toast]);
  ...
  const handleSave = async (publishStatus: 'Published' | 'Draft') => {
    if (!post || !title || !category || !content || !featuredImage) {
        toast({
    ...
    try {
        ...
        toast({
          title: `Post ${publishStatus === 'Published' ? 'Published' : 'Saved'}`,
        ...
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-admin-muted-foreground truncate max-w-sm">Editing: {post.title}</p>
                </div>
            </div>
