...
export default function CompressImagePage() {
  ...
  const handleDownloadAll = async () => {
    ...
    const results = await Promise.all(compressedFilesPromises);
    const filesToDownload = results.filter((f): f is ImageFile => f !== null && f.isCompressed);

    setIsProcessingAll(false);

    if (filesToDownload.length === 0) {
      toast({ title: "Nothing to download", description: "Compression might have failed for all images.", variant: "destructive" });
      return;
    }
    ...
    if (filesToDownload.length === 1) {
      ...
      if (file.compressedDataUrl) {
        ...
        await sessionStorage.setItem('compress-image-filename', file.file.name.replace(/(\.[^/.]+)/i, `_compressed.${outputFormat}`));
                ...
                {activeImage && (
                  allFilesCompressed && activeImage.compressedUrl ? (
                    ...
                    <>
                      ...
                      <div className='absolute top-2 left-2 bg-black/50 text-white px-2 py-1 rounded-md text-xs z-10'>
                ...
                {allFilesCompressed ? (
                  <>
                    <Button onClick={handleDownloadAll} size="lg" className='w-full h-12 text-base' disabled={isProcessingAll}>
                      ...
                      {isProcessingAll ? 'Preparing...' : (files.length > 1 ? `Download All (.zip)` : `Download Image`)}
                    </Button>
                  ...
                  <Button onClick={handleDownloadAll} size="lg" className='w-full h-12 text-base' disabled={files.length === 0 || isProcessingAll}>
                    {isProcessingAll ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Compressing...</> : `Compress & Download (${files.length})`}
                  </Button>
                )}
              </div>
            </aside>
